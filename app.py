...
class CustomerServiceSystem:
    def __init__(self):
        ...
        print("DEEPGRAM_API_KEY", self.DEEPGRAM_API_KEY)
    ...
    def play_audio(self, audio_file):
        ...
        try:
            ...
            # This loop waits until playback is complete
    ...
    def handle_query(self, user_input):
        ...
        if "meeting" in user_input and self.conversation_state['state'] is None:
            ...
            return f"Available staff members are: {staff_names}. Please specify who you'd like to meet with."
...
# Create a global instance of the system
...
def start_call():
    ...
    global conversation_active, conversation_thread, conversation_messages
    
    if conversation_active:
        return jsonify({"status": "error", "message": "Call already in progress"})
    
    conversation_active = True
    conversation_messages = []
    
    # Add initial greeting
    greeting = f"Welcome to {service_system.store_info['store_name']}! How may I assist you today?"
    conversation_messages.append({"role": "assistant", "content": greeting})
    
    # Generate and play greeting
    audio_file = service_system.generate_speech(greeting)
    if audio_file:
        threading.Thread(target=service_system.play_audio, args=(audio_file,)).start()
    
    # Start conversation thread
    conversation_thread = threading.Thread(target=run_conversation_loop)
...
def end_call():
    ...
    if not conversation_active:
        return jsonify({"status": "error", "message": "No call in progress"})
    
    conversation_active = False
    
    # Add farewell message
    farewell = "Goodbye! Have a great day!"
    conversation_messages.append({"role": "assistant", "content": farewell})
    
    # Generate and play farewell
    audio_file = service_system.generate_speech(farewell)
    if audio_file:
        service_system.play_audio(audio_file)
...
def run_conversation_loop():
    """Background thread for handling the conversation."""
    global conversation_active, conversation_messages
    
    while conversation_active:
        # Get user input only after ensuring audio playback is complete
        user_input = service_system.get_voice_input()
        
        # Handle empty transcription
        if not user_input:
            # Add system message about not hearing anything
            empty_prompt = "Sorry, I didn't hear anything. Could you please speak again?"
            conversation_messages.append({"role": "system", "content": empty_prompt})
            
            # Generate and play the prompt
            audio_file = service_system.generate_speech(empty_prompt)
            if audio_file:
                # Play audio and wait for it to complete before continuing
                service_system.play_audio(audio_file)
        ...
        # Process user input
        response = service_system.handle_query(user_input)
        
        # Add assistant response to conversation
        conversation_messages.append({"role": "assistant", "content": response})
        
        # Generate and play response
        audio_file = service_system.generate_speech(response)
        if audio_file:
            # Play audio and wait for it to complete before continuing to next iteration
            service_system.play_audio(audio_file)
